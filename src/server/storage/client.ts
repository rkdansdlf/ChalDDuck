 ...
 *
 * ⚠️ **서비스 롤 키를 쓴다.** 이 앱은 Supabase Auth 를 쓰지 않아서(가입·로그인이 없는
 * 제품이다) 저장소의 RLS 가 판단할 사용자가 없다. 그래서 버킷을 비공개로 두고 **접근
 * 권한은 앱이 직접 확인한다** — 모든 저장소 호출 앞에 `requireSessionMember()` 와
 * "우리 팀 것인지" 확인이 붙는다.
 *
 * 그래서 이 모듈은 서버 전용이다. 서비스 롤 키가 브라우저로 나가면 저장소 전체가 열린다.
 */
...
export function storage() {
  ...
  if (!url || !key) {
    throw new Error(
      "SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 가 없습니다. `.env.example` 을 참고하세요.",
    );
  }
